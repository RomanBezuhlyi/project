	...
	<div class="modal" @click.self="$emit('close')">
		...
		<div class="container">
			<h3 class="modal__name">
				{{ step !== 3 ? 'Регістрація пошти' : 'Регістрація' }}
			</h3>
			<div v-if="step === 1">
				<h2 class="modal__action">Введіть свою адресу Email</h2>
				<Input
					class="modal__input"
					type="email"
					v-model="email"
			...
			<div v-else-if="step === 2">
				<h2 class="modal__action">На вашу адресу відправлено код</h2>
				<Input
					class="modal__input"
					type="code"
					v-model="code"
		...
		handleSubmit() {
			if (this.step === 1) {
				if (!this.isValidEmail) {
					this.isError = true
					this.showNotificationStepOne = true
					setTimeout(() => {
						this.isError = false
